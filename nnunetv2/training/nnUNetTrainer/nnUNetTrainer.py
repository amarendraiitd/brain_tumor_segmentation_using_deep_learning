...
class nnUNetTrainer(object):
    ...
    def initialize(self):
        if not self.was_initialized:
            ...
            if self._do_i_compile():
                # return False # adding to remove torch.compile error
    ...
    def plot_network_architecture(self):
        if self._do_i_compile():
            return False # added this myself
...
#         self.logger.log('lrs', self.optimizer.param_groups[0]['lr'], self.current_epoch)
    ...
    def on_train_epoch_start(self):
        ...
        if self.optimizer is not None:
            ...
            self.print_to_log_file(f"Current learning rate: {lr}")
            self.logger.log('lrs', self.optimizer.param_groups[0]['lr'], self.current_epoch)
        else:
            self.print_to_log_file("Warning: Optimizer is None. Skipping learning rate print.")
    ...
    def run_training(self):
        ...
        if self.optimizer is None:
            self.optimizer = torch.optim.SGD(self.network.parameters(), lr=0.01, momentum=0.99, weight_decay=3e-5)
            self.lr_scheduler = None  # Optional: Add a scheduler later if needed
