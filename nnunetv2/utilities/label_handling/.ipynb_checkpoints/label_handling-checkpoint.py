...
class LabelManager(object):
    def __init__(self, label_dict: dict, regions_class_order: Union[List[int], None], force_use_labels: bool = False,
        ...
        # label_dict = {int(k): v for k, v in label_dict.items()} # adding for debuging
    ...
    def _sanity_check(self, label_dict: dict):
        if not 'background' in label_dict.keys():
            # print("DEBUG: label_dict passed to LabelManager =", label_dict)
            raise RuntimeError('Background label not declared (remember that this should be label 0!)')
        # if 0 not in label_dict:
        #     print("DEBUG: label_dict passed to LabelManager =", label_dict)
        #     raise RuntimeError("Background label must be declared with key 0.")
        # bg_label = 0
        # if label_dict[0] != "background":
        #     raise RuntimeError(f"Key 0 must be assigned to 'background'. Found: {label_dict[0]}")
